...
# get distances from pipeline qc
syn.get("syn29531457", downloadLocation = "~/cnf-cell-lines/data")
...
Fastq files from RNA sequencing data in this [dataset](https://www.synapse.org/#!Synapse:syn29542674/datasets/) were processed using standardized processing tools and parameters as listed in [this link](https://sagebionetworks.jira.com/wiki/spaces/NPD/pages/2595913729/Nextflow+Data+Processing+Configuration).

First, let's take some of the QC data from the pipeline to plot the sample similarity (Euclidean distance, calculated with https://github.com/nf-core/rnaseq/blob/master/bin/deseq2_qc.r):


```{r sample_distance, echo=FALSE, eval=TRUE, results='hide', message=FALSE}

distances <- read_delim('~/cnf-cell-lines/data/deseq2.sample.dists.txt') %>% 
  dplyr::rename("28cNF"="X28NF", "i28cNF"="i28NF") %>% 
  mutate(sample = case_when(sample=="X28NF"~"28cNF", 
                            sample=="i28NF"~"i28cNF",
                            sample!="X28NF"|sample!="i28NF" ~ sample)) %>% 
  column_to_rownames("sample")

p <- pheatmap(distances, 
         color = viridis::mako(1000, direction = -1), 
         cellwidth = 15, 
         cellheight = 15,
...
pdf("../figures/rnaseq_euclidean_difference.pdf", height = 10, width = 10)
p
dev.off()
```


Then, the raw counts of transcripts identified in the files were analyzed as follows:
...
```{r TMM-LCPM normalization, eval=TRUE, echo=FALSE, results='hide', message=FALSE, warning=FALSE, fig.height = 20, fig.width = 10}
...
pdf("../figures/PCA_092622.pdf", height = 20, width = 20)
plot_data %>% 
     ggplot(aes(x=PC1,
               y=PC2)) + 
...
pdf("../figures/volcanoplot_cNF_icNF_092622.pdf", width = 20, height = 20)
...
pdf("../figures/DEG-heatmap_092622_4clusters.pdf", height = 20, width = 20)
heatmap_1
dev.off()

pdf("../figures/DEG-heatmap_092622_all_sig_genes_dendrogram.pdf", height = 20, width = 20)
plot(heatmap_1$tree_row, cex = 0.7)
abline(h=100, col="red", lty = 2, lwd = 2)
dev.off()

# dendrogram with the samples
pdf("../figures/DEG-heatmap_092622_all_samples_dendrogram.pdf", height = 20, width = 20)
...
```{r pathway-1, echo=F, eval=TRUE, fig.height=20, fig.width=20}
