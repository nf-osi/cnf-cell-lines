...
for i in *.vcf.gz;
do
    ic=${i%_VEP.ann.vcf.gz} 
    
    echo $ic
    bcftools filter $i -i 'FILTER="PASS" && %QUAL>20' -o DeepVariant_$ic.filtered.vcf.gz 
done

# for i in DeepVariant_*.filtered.vcf.gz
# do
#     bcftools sort $i -o $i
#     bcftools index $i
# done
...
for i in DeepVariant_*.filtered.vcf.gz
do
    synapse store $i --parentId syn35591895
done
